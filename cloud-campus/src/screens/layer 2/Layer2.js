import React from "react";

export const Layer2 = () => {
  return (
    <div className=" flex justify-center border-0 border-black py-2">
      <div className="border-0 w-[80%]">
        <div className="border-0 flex  flex-1 items-center flex-wrap">
          <div className="border-0  flex-1 flex flex-row justify-center">
            <div>
              <p className="text-[100%] font-bold border-0 text-blue-400">
              Apprenez comment vous voulez, où vous voulez
              </p>
              <p className="text-[200%] font-bold border-0">
              La plus grande sélection de cours en ligne au monde
              </p>

              <p className="text-[100%] font-bold border-0 py-3 text-gray-500">
              Des millions de personnes ont utilisé Kingster pour décider lequel en ligne
                cours à suivre. Nous regroupons les cours de nombreuses universités pour
                vous aider à trouver les meilleurs cours sur presque tous les sujets, où que vous soyez
                ils existent. Notre objectif est de faire fonctionner l'éducation en ligne pour
                tout le monde.
              </p>

              <div className="flex border-0 flex-wrap justify-center">
                <div className="flex p-4">
                  <img
                    alt="icon1"
                    src="data:image/svg+xml;base64,"
                  />
                  <p className="pl-2 font-bold">Classes privées</p>
                </div>

                <div className="flex p-4">
                  <img
                    alt="icon2"
                    src="data:image/svg+xml;base64,"
                  />
                  <p className="pl-2 font-bold">Petits groupes</p>
                </div>

                <div className="flex p-4">
                  <img
                    alt="icon3"
                    src="data:image/svg+xml;base64,"
                  />
                  <p className="pl-2 font-bold">Accès à vie</p>
                </div>

                <div className="flex p-4">
                  <img
                    alt="icon4"
                    src="data:image/svg+xml;base64,"
                  />
                  <p className="pl-2 font-bold">Tutorat en ligne</p>
                </div>
              </div>
            </div>
          </div>

          <div className="border-2 flex-1 flex flex-row justify-center flex-wrap">
            <div className="border-2 ">
              <img
                alt="cloud campus"
                width={500}
                src="https://media.istockphoto.com/photos/focused-cute-stylish-african-american-female-student-with-afro-picture-id1311107708?b=1&k=20&m=1311107708&s=170667a&w=0&h=9I2i47JWO0Tof4wbomb9kRxoOXJZJvS8awg6NHFDHco="
              />
            </div>
            <div className="border-2 ">
              <img
                alt="cloud campus"
                width={500}
                src="https://media.istockphoto.com/photos/man-working-picture-id1359589480?b=1&k=20&m=1359589480&s=170667a&w=0&h=yclCcqC7c1KbKSemrAQowp20lGafkR2k-Dlv9OBmEL8="
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
