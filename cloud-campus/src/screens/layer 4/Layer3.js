import React from "react";

export const Layer3 = () => {
  return (
    <div className="h-screen flex justify-center">
      <div className="border-0 w-[80%]">
        <div className="border-0 flex  h-screen items-center flex-wrap">


          <div className="border-2 flex-1 flex flex-row justify-center flex-wrap">
            <div className="border-2 ">
              <img
                alt="cloud campus"
                width={500}
                src="https://media.istockphoto.com/photos/focused-cute-stylish-african-american-female-student-with-afro-picture-id1311107708?b=1&k=20&m=1311107708&s=170667a&w=0&h=9I2i47JWO0Tof4wbomb9kRxoOXJZJvS8awg6NHFDHco="
              />
            </div>
            <div className="border-2 ">
              <img
                alt="cloud campus"
                width={500}
                src="https://media.istockphoto.com/photos/man-working-picture-id1359589480?b=1&k=20&m=1359589480&s=170667a&w=0&h=yclCcqC7c1KbKSemrAQowp20lGafkR2k-Dlv9OBmEL8="
              />
            </div>
          </div>

          <div className="border-0  flex-1 flex flex-row justify-center">
            <div>
              <p className="text-[100%] font-bold border-0 text-blue-400">
              NEW ADDITIONS PUBLISHED EVERY MONTH
              </p>
              <p className="text-[200%] font-bold border-0">
              Choose from over 100,000 online video courses
              </p>

            

              <div className="flex border-0 flex-col justify-center py-5">
                <div className="flex p-4">
                  <img
                    alt="icon1"
                    width={40}
                    src="data:image/svg+xml;base64,"
                  />
                  <div>
                    <p className="pl-2 font-bold">Certificates</p>
                    <p className="pl-2 text-gray-800">Earn official recognition for your work.</p>
                  </div>
                </div>

                <div className="flex p-4">
                  <img
                    alt="icon2"
                    width={40}
                    src="data:image/svg+xml;base64,"
                  />
                   <div>
                    <p className="pl-2 font-bold">Coursework</p>
                    <p className="pl-2 text-gray-800">Each course is like an interactive textbook.</p>
                  </div>
                </div>

                <div className="flex p-4">
                  <img
                    alt="icon3"
                    width={40}
                    src="data:image/svg+xml;base64,"
                  />
                   <div>
                    <p className="pl-2 font-bold">Support</p>
                    <p className="pl-2 text-gray-800">Connect with thousands of other learners.</p>
                  </div>
                </div>

               
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
